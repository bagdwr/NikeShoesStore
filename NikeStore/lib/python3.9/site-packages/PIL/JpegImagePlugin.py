...
# 1997-08-27 fl   Save mode 1 pictures as BW (0.3)
...
def SOF(self, marker):
    ...
    if self.layers == 1:
        ...
        raise SyntaxError(f"cannot handle {self.layers}-layer pictures")
...
##
# Image plugin for JPEG and JFIF pictures.
...
def _getmp(self):
    ...
    # it's an error not to have a number of pictures
    try:
        quant = mp[0xB001]
    except KeyError as e:
        raise SyntaxError("malformed MP Index (no number of pictures)") from e
    # get MP entries
    mpentries = []
    try:
...
def get_sampling(im):
    # There's no subsampling when pictures have only 1 layer
    # (grayscale pictures) or when they are CMYK (4 layers),
